  ...
  virtual void play_sound(int id, xy position, const unit_t* source_unit, bool add_race_index) override {
    ...
    */
    ...
    if (volume > 10) {
      ...
      if (sound_type->flags & 0x10) {
        ...
        for (auto& c : global_ui_st.sound_channels) {
          if (c.playing && c.sound_type == sound_type) {
            if (native_sound::is_playing(i)) return;
            c.playing = false;
          }
          i++;
        }
      ...
      if (c) {
        native_sound::play(int(c - global_ui_st.sound_channels.data()), &*s, (128 - 4) * (volume * global_ui_st.global_volume / 100) / 100, pan);
        c->playing = true;
  ...
  void draw_tiles(uint8_t* data, size_t data_pitch, rect screen_rect) {
    ...
    for (size_t tile_y = screen_tile.from.y; tile_y != screen_tile.to.y; ++tile_y) {
      for (size_t tile_x = screen_tile.from.x; tile_x != screen_tile.to.x; ++tile_x) {
        ...
        // Draw creep edges
        if (!draw_creep_here) {
          std::uint32_t creep_index = 0;
          for (std::uint32_t i = 0; i < 8; ++i) {
            int add_x = dirs[i].x;
            int add_y = dirs[i].y;
            if (tile_x + add_x >= game_st.map_tile_width) continue;
            if (tile_y + add_y >= game_st.map_tile_height) continue;
            if (st.draw_creep_over[tile_x + add_x + (tile_y + add_y) * game_st.map_tile_width]) {
              creep_index |= std::uint32_t(1) << i;
            }
          }
